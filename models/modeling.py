...
class MyBertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        head_hidden_size = int(config.hidden_size/config.num_attention_heads)
    ...
    def forward(
        ...
        max_n = query_layer.size()[2]
        query_layer = query_layer.unsqueeze(3).repeat(1,1,1,max_n,1)
        key_layer = key_layer.unsqueeze(3).repeat(1,1,1,max_n,1).transpose(2,3)
        q_k_cat = torch.cat([query_layer,key_layer],dim=-1)
        attention_scores = self.mlp(q_k_cat).squeeze()
